...
`scfetch` provides `ExtractCBDatasets` to extract metadata with collection, sub-collection, organ, disease status, organism, project and cell number. All attributes except cell number support fuzzy match with `fuzzy.match`, this is useful when selecting datasets.
...
`scfetch` provides `ExtractCELLxGENEMeta` to extract sample metadata, including sample title, description, contact, organism, ethnicity, sex, tissue, disease, assay, suspension type, cell type, et al.
