...
Since [PanglaoDB](https://panglaodb.se/about.html) is no longer maintained, `scfetch` has cached all metadata and sample composition and use these cached data by default to accelerate, users can access the cached data with `PanglaoDBMeta` (all metadata) and `PanglaoDBComposition` (all sample composition).
...
`scfetch` uses cached metadata and sample composition by default, users can change this by setting `local.data = FALSE`.
...
`scfetch` provides `ExtractCELLxGENEMeta` to filter sample metadata, the available values of attributes can be obtained with `StatDBAttribute` except **cell number**:
