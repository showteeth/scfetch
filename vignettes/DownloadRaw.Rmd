...
With the dataframe contains gsm and run number, `scfetch` will download sra files using `prefetch`:
...
GSE130636.split <- SplitSRA(
  ...
  sratools.path = "/usr/local/bin", split.cmd.paras = "--split-files --gzip",
  split.cmd.threads = 4
)
```
...
This is an example to download 10x generated bam file:
