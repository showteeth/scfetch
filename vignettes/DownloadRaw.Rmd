...
A common situation is that we need to use a unified software version (e.g. CellRanger) to obtain the count matrix, in order to better integrate and compare multiple datasets. Here, we will use `GEfetch2R` to download fastq and bam files. With bam files, `GEfetch2R` also provides function for user to convert the bam to fastq.
